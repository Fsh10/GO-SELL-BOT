...
import (
	...
	"github.com/Fsh10/GO-SELL-BOT/bot/countries"
	"github.com/Fsh10/GO-SELL-BOT/bot/desc"
	"github.com/Fsh10/GO-SELL-BOT/bot/digi"
	"github.com/Fsh10/GO-SELL-BOT/bot/search"
	"go.uber.org/zap"
)
...
func (b *bot) ProductCallback(upd tgbotapi.Update, productsEntity callbackEntity) {
	...
	rows := tgbotapi.NewInlineKeyboardRow(
		tgbotapi.NewInlineKeyboardButtonURL("Buy", product.PaymentURL(b.opts.sellerId)),
	)
	if product.AddInfo != "" {
		data := callbackEntity{
			parentType: Product,
		...
		button := tgbotapi.NewInlineKeyboardButtonData("Instruction", marshallCb(data))
		rows = append(rows, tgbotapi.NewInlineKeyboardRow(button)...)
	}
		...
		if price, ok := product.Prices[country.Currency]; ok && price != 0 {
			...
			if curentPrice == 0 ||
				...
				if botProduct.PurchaseType == purchaseTypeAcc {
					buttonText = fmt.Sprintf(`"Purchase on account" %s for %d rub.`, country.Flag, rubPrice)
				} else if botProduct.PurchaseType == purchaseTypeKey {
					buttonText = fmt.Sprintf(`"Activation key" %s for %d rub.`, country.Flag, rubPrice)
				} else {
					continue
				}
